...
//             Separate from QAppNavShell by design — the control plane has its
//             own layout contract: no collapse, group headers, breadcrumb bar.
//             Uses kNavTemplateAdmin internally; accepts a QNavTemplate for
//             overrides (e.g. disabling group headers for a specific client).
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//
// Usage (in QAdminShell — lib/core/admin/q_admin_shell.dart):
//
//   QAdminNav(
...
class QAdminNav extends StatefulWidget {
  /// Grouped nav items. The admin sidebar organizes by section (Content,
  /// Brand, etc.). Pass flat items wrapped in a single QNavGroup if grouping
  /// isn't needed.
  ...
  /// Page title shown in the top breadcrumb strip.
  final String breadcrumb;

  /// Optional — actions for the top strip (publish button, etc.).
  final Widget? topStripTrailing;

  /// Template — defaults to kNavTemplateAdmin. Override to tweak group
  /// headers or other admin-specific flags.
...
class _QAdminNavState extends State<QAdminNav> {
  ...
  Widget build(BuildContext context) {
    // Admin always uses sidebar on wide screens, drawer on narrow.
    // Bottom nav is not used for the control plane — admins use a browser.
    ...
    return QNavScope(
      ...
      sidebarExpanded: true, // admin sidebar is always expanded
      toggleSidebar:   () {},
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
            ...
            Divider(height: 1, color: AppColors.border),
            // Admin footer — version tag or env label
            Padding(
...
// ─────────────────────────────────────────────────────────────────────────────
// _AdminTopStrip — breadcrumb bar + trailing actions
// ─────────────────────────────────────────────────────────────────────────────
